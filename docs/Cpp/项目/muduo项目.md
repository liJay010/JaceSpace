...
​		这个项⽬是我在学习计算机⽹络和Linux socket编程过程中根据muduo重写的Web服务器，服务器的⽹络模型是主从reactor加线程池的模式，IO处理使⽤了⾮阻塞IO和IO多路复⽤技术。项⽬中的⼯作可以分为两部分，⼀部分是服务器⽹络框架、⽇志系统、存储引擎等⼀些基本系统的搭建，另⼀部分是为了提⾼服务器性能所做的⼀些优化，⽐如缓存机制、内存池等⼀些额外系统的搭建。最后还对系统中的部分功能进⾏了功能和压⼒测试。对于存储引擎的压⼒测试，在本地测试下，存储引擎读操作的QPS可以达到36万，写操作的QPS可以达到30万。对于⽹络框架的测试，使⽤webbench创建1000个进程对服务器进⾏60s并发请求，测试结果表明，对于短连接的QPS为1.8万，对于⻓连接的QPS为5.2万。在开发阶段对于⼀些功能的不同实现⽅法，我先思考⼀下各个⽅法的应⽤场景和效率，再进⾏选择，通过这个项⽬，我学习了两种Linux下的⾼性能⽹络模式，熟悉了Linux环境下的编程，并在后期优化的过程中了解了⼀些服务端性能调优的⽅法。
...
2. 另⼀部分是为了提⾼服务器性能所做的⼀些优化，⽐如缓存机制、内存池等⼀些额外系统的搭建。这部分的难
...
4. **【经纬恒润⼆⾯】**服务器的功能？（HTTP，FTP，存储）为什么要做这么⼀个项⽬？（balabala）

5. **【地平线⼀⾯】**为什么要做web服务器这个项⽬（热爱编程，巩固所学知识balabla），有参考开源项⽬吗，做了哪些优化？

6. **【地平线⼆⾯】**讲⼀下WebServer中你和别⼈不⼀样的地⽅？（主要讲了下⾃⼰为了优化服务端性能所做的⼀些⼯作）

7. **【荣耀⼆⾯】**介绍项⽬？讲⼀下⾃⼰的项⽬中和别⼈不⼀样的部分？项⽬有部署上线吗？（⾃⼰租了个阿⾥云服务器，在上⾯部署过）

8. **【华为⼀⾯】**简要介绍项⽬？为什么要做这么⼀个项⽬？和别⼈不⼀样的地⽅？（增加了对服务端性能的⼀些优化⼿段）对服务端的性能优化体现在哪？⽤哪些指标进⾏评价？是如何进⾏测量的？你从这个项⽬中学到了什么，或者说有什么收获？

9. **【华为⼆⾯】**为什么要做WebServer这个项⽬？（学习⽤的）计算机相关的知识你是怎么进⾏学习的？

10. **【**oppo**⼀⾯】**介绍项⽬整体框架（介绍主从reactor+线程池模式）这个框架是你设计的吗？（不是，参考muduo的，然后介绍了另外⼏种⽹络框架），有参考过其他⼈的实现吗（有）你做的⼯作和其他⼈有什么不⼀样？（为了提⾼服务器效率加⼊了内存池和缓存机制，同时可以对外提供存储服务）

11. **【**oppo**⼆⾯】**为什么做WebServer这么⼀个项⽬，和你专业也不搭，跨度有点⼤（balabala）介绍⼀下WebServer的功能，⽹络框架是怎么样的，监听多少个端⼝（HTTP，FTP，存储引擎），怎么监听（epoll），介绍⼀下epoll。⽤的什么语⾔？项⽬中如何体现你的代码规范？（从模块解耦、命名空间、命名代码随想录知识星球规范等去讲）

12. **【远景智能三⾯】**很多⼈的简历上都有这个项⽬，你做的这个项⽬有什么和其他⼈不⼀样的地⽅吗？（balabala）刚才提到做了⼀些性能⽅⾯的优化？那对于服务端的性能优化你是怎么理解的？

13. **【蔚来⼆⾯】**很多⼈的简历上都会有这个项⽬，为什么你还要选择？（项⽬是在学习计⽹的过程中逐步搭建的，这个项⽬综合性⽐较强，从中既能学习Linux环境下的⼀些系统调⽤，也能熟悉⽹络编程和⼀些⽹络框架，其中也根据⾃⼰的理解加⼊了⼀些性能调优的⼿段）讲⼀下你刚才讲的那些性能调优的⼿段？（从程序本身的优化和系统参数的优化两⽅⾯去讲）

14. **【蔚来⼆⾯】**为了减少系统调⽤，你具体做了哪些⼯作？（分类讲：线程池、内存池、缓存机制；零拷⻉；读写锁）

15. **【远景智能三⾯】**为什么想到去做这么⼀个项⽬？你从中学到了什么？这个项⽬做下来是怎么⼀个流程？

16. **【经纬恒润⼆⾯】**说⼀下你这个WebServer的整体运⾏流程？（对着ppt讲）你这个框架的优点是什么？（I/O任务和计算任务解耦，避免计算密集型连接占⽤subReactor导致⽆法响应其他连接，可扩展性好）还了解哪些其他⽹络框架？（Proactor）
...
1. **【中兴⼀⾯】**讲⼀下你项⽬中缓存池的作⽤？为什么要选择 LFU ⽽不是 LRU？LFU有什么缺点？（最近加⼊的数据因为起始的频率很低，容易被淘汰，⽽早期的热点数据会⼀直占据缓存）可以进⾏优化吗？（LFUaging，或者使⽤LRU+LFU的形式）

2. **【蔚来⼆⾯】**缓存机制为什么选⽤LFU？（主要考虑热点⻚⾯）热点⻚⾯指的什么？怎么知道是热点⻚⾯？有考虑过其他的缓存算法吗？（LRU，ARC）为什么不选⽤？（balabala）LFU什么缺陷吗，可以怎么进⾏优化？（LFU-aging，window-LFU，ARC）

3. **【经纬恒润⼀⾯】**为什么⽤LFU，不⽤LRU？（保证热点⻚⾯的响应）LFU的实现可以讲⼀下吗？（双链表+双哈希）

4. **【**oppo**⼀⾯】**讲⼀讲为什么要加⼊缓存机制，常⽤的缓存算法有哪些？LFU的缺点是什么？怎么优化？（LFU-aging）
