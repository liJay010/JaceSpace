...
public:
    int numDecodings(string s) {
        ...
        dp[1] = (res[0] * 10 + res[1] <= 26 && res[0] < 10 && res[1] < 10) ? 2 : 1;

        for (int i = 2; i < res.size(); ++i) {
            dp[i] = ((res[i-1] * 10 + res[i]) <= 26 && res[i-1] < 10 && res[i] < 10) ? dp[i-1] + dp[i-2] : dp[i-1];
        }
        return dp.back();
    }
};
...
### [650. 只有两个键的键盘](https://leetcode.cn/problems/2-keys-keyboard/)（未完成）
...
### [413. 等差数列划分](https://leetcode.cn/problems/arithmetic-slices/)（未完成）
...
### [10. 正则表达式匹配](https://leetcode.cn/problems/regular-expression-matching/)（未完成）
