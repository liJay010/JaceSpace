...
### [81. 搜索旋转排序数组 II](https://leetcode.cn/problems/search-in-rotated-sorted-array-ii/)（未完成）
...
public:
    vector<vector<int>> matrixReshape(vector<vector<int>>& mat, int r, int c) {
        ...
        for (int i = 0; i < r; ++i) {
            for (int j = 0; j < c; ++j) {
                int ri = cnt / mat[0].size() , rj =  cnt % mat[0].size() ;
                res[i][j] = mat[ri][rj];
                cnt++;
            }
        }
...
public:
    ListNode* sortList(ListNode* head) {
        ...
        if (left) presss->next = left;
        if (right) presss->next = right;
        return res->next;
    }
};
```
