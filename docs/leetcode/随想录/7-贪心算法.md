...
public:
    vector<int> advantageCount(vector<int>& nums1, vector<int>& nums2) {
        ...
        for (int i = 0; i < nums2.size(); ++i) {
            ...
            int nofind = INT_MAX;
            for (auto x:umap) {
                if (x.first > nums2[i]) mins = min(mins,x.first -nums2[i]);
                nofind=min(nofind,x.first);
            }
            if (mins!=INT_MAX)
            {
                res.push_back(nums2[i] + mins);
                if(--umap[nums2[i] + mins] == 0) umap.erase(nums2[i] + mins);
            } else
            {
                res.push_back(nofind);
                if(--umap[nofind] == 0) umap.erase(nofind);
            }
